...
export function AuthProvider({ children }) {
    ...
    const [currentUser, setCurrentUser] = useState(localStorage.getItem("username")); // State to hold the current user's information.
    const[loginError, setLoginError] = useState(null); // State to hold any login error messages.
    const VALID_USERNAME = "jenna"; // Predefined valid username.
    const VALID_PASSWORD = "lee"; // Predefined valid password.
    ...
    const login = (username, password) => {
        if(username === VALID_USERNAME && password === VALID_PASSWORD){
            ...
            navigate("/"); //take them to main page 
        } else{
    ...
    const logout = () => {
        ...
        navigate("/login"); //take them to login page
    };
